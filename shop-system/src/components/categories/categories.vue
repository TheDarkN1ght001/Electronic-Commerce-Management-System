<template>
  <el-card>
    ...
    <!-- 表格 -->
    <el-table v-loading="loading" :data="pageList" style="width: 99%">
    ...
    <el-dialog title="编辑商品分类" :visible.sync="editdialog">
      ...
      <el-form>
          {{editMsg}}{{catid}}
        <el-form-item label="分类名称" :label-width="formLabelWidth">
          <el-input  v-model="editMsg" autocomplete="off"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click.prevent="editvisnone">取 消</el-button>
        <el-button @click.prevent="editMsgFn(catid)" type="primary">确 定</el-button>
      </div>
    </el-dialog>
  </el-card>
</template>
